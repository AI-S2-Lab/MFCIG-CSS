...
from torch_geometric.nn import HGTConv, Linear, HeteroConv, SAGEConv, GCNConv
...
class VarianceAdaptor(nn.Module):
    ...
    def forward(
        ...
        # x_dur = x.clone()
        if I_s is not None:
            x = x + I_s.unsqueeze(1).expand(
                -1, text.shape[1], -1
            )

        if I_p is not None:
            x = x + I_p.unsqueeze(1).expand(
                -1, text.shape[1], -1
            )
...
class SemanticInteractionGraph(nn.Module):
    def __init__(self):
        super(SemanticInteractionGraph, self).__init__()
        self.text_sen_linear = nn.Linear(1024, 256)
        self.text_word_linear = nn.Linear(768, 256)
        self.audio_word_linear = nn.Linear(768, 256)
        self.heteroConv = HeteroConv({
            ('text_word', 'next', 'text_sen'): SAGEConv(256, 256),
            ('audio_word', 'next', 'text_sen'): SAGEConv(256, 256),
            ('text_sen', 'next', 'text_sen'): SAGEConv(256, 256),
        }, aggr='mean')



    def forward(self, graph_list):
        graph_list_batch = Batch.from_data_list(graph_list)
        x_dict = graph_list_batch.x_dict
        edge_index_dict = graph_list_batch.edge_index_dict
        x_dict['text_sen'] = self.text_sen_linear(x_dict['text_sen'])
        x_dict['text_word'] = self.text_word_linear(x_dict['text_word'])
        x_dict['audio_word'] = self.audio_word_linear(x_dict['audio_word'])

        x_dict_enc = self.heteroConv(x_dict, edge_index_dict)
        x_dict['text_sen'] = x_dict_enc['text_sen']

        graph_list_batch.x_dict = x_dict
        graph_list_batch['text_sen'].x = x_dict_enc['text_sen']
        graph_list_batch_spl = graph_list_batch.to_data_list()
        graph_list_batch_spl_x = [item['text_sen'].x for item in graph_list_batch_spl]

        seven = [torch.mean(item, dim=0) for item in graph_list_batch_spl_x]
        I_s = torch.stack(seven)

        return  I_s


class ProsodyInteractionGraph(nn.Module):
    def __init__(self):
        super(ProsodyInteractionGraph, self).__init__()
        self.audio_sen_linear = nn.Linear(1280, 256)
        self.text_word_linear = nn.Linear(768, 256)
        self.audio_word_linear = nn.Linear(768, 256)
        self.heteroConv = HeteroConv({
            ('text_word', 'next', 'audio_sen'): SAGEConv(256, 256),
            ('audio_word', 'next', 'audio_sen'): SAGEConv(256, 256),
            ('audio_sen', 'next', 'audio_sen'): SAGEConv(256, 256),
        }, aggr='mean')
    ...
    def forward(self, graph_list):
        ...
        x_dict = graph_list_batch.x_dict
        edge_index_dict = graph_list_batch.edge_index_dict
        x_dict['audio_sen'] = self.audio_sen_linear(x_dict['audio_sen'])
        x_dict['text_word'] = self.text_word_linear(x_dict['text_word'])
        x_dict['audio_word'] = self.audio_word_linear(x_dict['audio_word'])
        x_dict_enc = self.heteroConv(x_dict, edge_index_dict)
        x_dict['audio_sen'] = x_dict_enc['audio_sen'] 

        graph_list_batch.x_dict = x_dict
        graph_list_batch['audio_sen'].x = x_dict_enc['audio_sen']
        graph_list_batch_spl = graph_list_batch.to_data_list()
        graph_list_batch_spl_x = [item['audio_sen'].x for item in graph_list_batch_spl]

        seven = [torch.mean(item, dim=0) for item in graph_list_batch_spl_x]
        I_p = torch.stack(seven)
