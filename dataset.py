...
class Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        history_audio_word_emb = list()
        history_text_word_emb = list()
        history_speaker_emb = list()
        history_word_lens = list()
        ...
        if self.history_type != "none":
            ...
            if self.history_type == "Guo":
                ...
                for i, h_basename in enumerate(history_basenames):
                    ...
                    h_spk_emb_path = os.path.join(
                        self.preprocessed_path,
                        "x_vectors",
                        "{}.npy".format(spk_flg),
                    )
                    h_spk_emb = np.load(h_spk_emb_path)

                    history_speaker_emb.append(h_spk_emb.squeeze())
                    ...
                    h_text_word_emb_path = os.path.join(
                        self.preprocessed_path,
                        "text_words_tod_emb",
                        "{}-text_words_tod_emb-{}.npy".format(h_speaker, h_basename),
                    )
                    h_text_word_emb = np.load(h_text_word_emb_path)
                    history_text_word_emb.append(h_text_word_emb)
                    ...
                    h_audio_word_emb_path = os.path.join(
                        self.preprocessed_path,
                        "audio_word_emb",
                        "{}-audio_word_emb-{}.npy".format(h_speaker, h_basename),
                    )
                    h_audio_word_emb = np.load(h_audio_word_emb_path)
                    history_audio_word_emb.append(np.squeeze(h_audio_word_emb, axis=1))

                    history_word_lens.append(h_text_word_emb.shape[0])
                    ...
                    #     )
                if turn == 0:
                    history_speaker_emb.append(np.zeros(512))
                    history_text_emb.append(np.zeros(512))
                    history_audio_emb.append(np.zeros(768))
                    history_text_word_emb.append(np.zeros((1, 768)))
                    history_audio_word_emb.append(np.zeros((1, 768)))
                word_lens = [item.shape[0] for item in history_text_word_emb]


                history_speaker_emb = torch.from_numpy(np.array(history_speaker_emb))
                history_text_emb = torch.from_numpy(np.array(history_text_emb))
                history_text_emb = torch.cat([history_text_emb, history_speaker_emb], dim=-1)
                history_audio_emb = torch.from_numpy(np.array(history_audio_emb))
                history_audio_emb = torch.cat([history_audio_emb, history_speaker_emb], dim=-1)
                history_text_word_emb = torch.from_numpy(np.concatenate(history_text_word_emb, axis=0))
                history_audio_word_emb = torch.from_numpy(np.concatenate(history_audio_word_emb, axis=0))

                sen_next_text_sen = []
                for i in range(history_text_emb.shape[0]):
                    sen_next_text_sen.append([i, i+1])

                word_next_text_sen = []
                word_idx = 0
                for i, num in enumerate(word_lens):
                    for j in range(num):
                        word_next_text_sen.append([word_idx, i+1])
                        word_idx+=1



                semantic_graph = HeteroData()
                semantic_graph['text_sen'], semantic_graph['text_word'], semantic_graph['audio_word']
                semantic_graph['text_sen', 'next', 'text_sen']
                semantic_graph['text_word', 'next', 'text_sen']
                semantic_graph['audio_word', 'next', 'text_sen']
                zero_vector1 = torch.zeros(1, 1024)
                history_text_emb = torch.cat([history_text_emb, zero_vector1], dim=0)

                semantic_graph['text_sen'].x = history_text_emb
                semantic_graph['text_word'].x = history_text_word_emb
                semantic_graph['audio_word'].x = history_audio_word_emb

                semantic_graph['text_sen', 'next', 'text_sen'].edge_index = torch.tensor(
                    sen_next_text_sen).contiguous().transpose(-2, -1)
                semantic_graph['text_word', 'next', 'text_sen'].edge_index = torch.tensor(
                    word_next_text_sen).contiguous().transpose(-2, -1)
                semantic_graph['audio_word', 'next', 'text_sen'].edge_index = torch.tensor(
                    word_next_text_sen).contiguous().transpose(-2, -1)

                prosody_graph = HeteroData()
                prosody_graph['audio_sen'], prosody_graph['text_word'], prosody_graph['audio_word']
                prosody_graph['audio_sen', 'next', 'audio_sen']
                prosody_graph['text_word', 'next', 'audio_sen']
                prosody_graph['audio_word', 'next', 'audio_sen']

                zero_vector2 = torch.zeros(1, 1280)
                history_audio_emb = torch.cat([history_audio_emb, zero_vector2], dim=0)

                prosody_graph['audio_sen'].x = history_audio_emb
                prosody_graph['text_word'].x = history_text_word_emb
                prosody_graph['audio_word'].x = history_audio_word_emb

                prosody_graph['audio_sen', 'next', 'audio_sen'].edge_index = torch.tensor(
                    sen_next_text_sen).contiguous().transpose(-2, -1)
                prosody_graph['text_word', 'next', 'audio_sen'].edge_index = torch.tensor(
                    word_next_text_sen).contiguous().transpose(-2, -1)
                prosody_graph['audio_word', 'next', 'audio_sen'].edge_index = torch.tensor(
                    word_next_text_sen).contiguous().transpose(-2, -1)

                history = {
                    "semantic_graph": semantic_graph,
                    "prosody_graph": prosody_graph,
                }
    ...
    def reprocess(self, data, idxs):
        ...
        if self.history_type != "none":
            if self.history_type == "Guo":
                semantic_graph_list = [data[idx]["history"]["semantic_graph"] for idx in idxs]
                prosody_graph_list = [data[idx]["history"]["prosody_graph"] for idx in idxs]
    ...
    def collate_fn(self, data):
        ...
        if self.history_type != "none":
            if self.history_type == "Guo":
                ...
                semantic_graph_list = [d[6]["semantic_graph"] for d in data]
                prosody_graph_list = [d[6]["prosody_graph"] for d in data]
